...
const file = ref('')
const background_chat = 'http://localhost:8081/uploads/translucent-image.png'
const AllTime = ref('')
...
const SendMEss = () => {
  ...
  if (file.value) {
    ...
    .catch(error => {
      ...
      .then(response => {
        if (response.ok) {
          ...
          .then(response => {
              ...
              fetch('http://localhost:8081/uploads_video', { 
                method: 'POST',
                body: formData
              })
              .then(response => {
      ...
      fetch('http://localhost:8081/uploads_file', { 
        method: 'POST',
        body: formData
      })
      .then(response => {
...
<div class="chat-frame-empty" v-if="UserId === ''">Search Chat</div>
  <div :class="['chat-frame', {'chat-frame_d': Number(user.id_chat) === UserId}]" v-for="user in MyUsers" :key="user.id">
    <div class="chat-frame__wrapper" :style="'background: url(' + background_chat + '); background-size: 510px auto'">
      ...
      <div class="chat__content" >
        <div class="chat__messages" v-for="mess in Message" :key="mess.chat_id">
          ...
          >
            ...
            <div @click="FullImg.path = `http://localhost:8081/uploads/` + mess.main.img" class="chat__img" v-if="mess.main.img"> <img class="chat-img" @click="FullImg.open = true" :src="`http://localhost:8081/uploads/` + mess.main.img" alt="img"><br>
            <span class="chat__time">{{mess.main.time}}</span>
            </div>
            <div class="chat__video" v-if="mess.main.video"> <video controls class="chat-img"> <source :src="`http://localhost:8081/uploads_video/` + mess.main.video" type="video/mp4" />  </video><br>
            <span class="chat__time">{{mess.main.time}}</span>
            </div>
            <div class="chat__file" v-if="mess.main.file">
              <a :href="'http://localhost:8081/uploads_file/' + mess.main.file" class="chat__file-icon" download><ion-icon name="arrow-down-outline"></ion-icon></a>
            ...
            <div class="chat__voice" v-if="mess.main.voice">
              <button ref="VoicePlay" style="display: flex;" class="chat__voice-controls" @click="play('http://localhost:8081/uploads_file/' + mess.main.voice)" :href="'http://localhost:8081/uploads_file/' + mess.main.voice"><ion-icon  name="play"></ion-icon></button>
              <button ref="VoiceStop" style="display: none;" class="chat__voice-controls" @click="stop('http://localhost:8081/uploads_file/' + mess.main.voice)" :href="'http://localhost:8081/uploads_file/' + mess.main.voice"><ion-icon  name="pause"></ion-icon></button>
              <!-- <div ref="CurrentTime" :href="'http://localhost:8081/uploads_file/' + mess.main.voice" class="chat__voice-time">0</div> -->
              <div class="chat__voice-main">
                <div class="chat__voice-progress">
                  <div style="background: rgb(50, 108, 242); width: 0;" class="progress" ref="progress" :href="'http://localhost:8081/uploads_file/' + mess.main.voice" ></div>
                </div>
                <div class="chat__voice-times">
                  <div ref="AllTime" :href="'http://localhost:8081/uploads_file/' + mess.main.voice" class="chat__voice-time">00:00</div>
                  <span class="chat__time">{{mess.main.time}}</span>
                </div>
              ...
              <audio style="display: none;" ref="AudioElm" :src="'http://localhost:8081/uploads_file/' + mess.main.voice">                
              </audio>
