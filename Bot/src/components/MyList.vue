...
socket.on('connect', () => {
    axios
      .put('http://localhost:8081/api/bot_update_list/',
      {
...
const GetUserMess = () => {
  fetch('http://localhost:8081/api/mess/' + TrueId.value)
...
socket.on('receive-mess-view', (token, user_id, count) => {
  ...
  if (MessView.value.includes(Number(TrueId.value))) {
    ...
    axios
      .put('http://localhost:8081/api/mess/' + TrueId.value,
      {
        seen: true,
...
onMounted(() => {
  fetch('http://localhost:8081/api/bot-users/' + props.token)
    .then((response) => response.json())
    .then((data) => {
      Users.value = data
      SendUser()
  ...
  <div class="users" @click="SendId">
    ...
    <div class="users__list">
      <div class="users__item" v-for="user in SortedUsers" :key="user.id" @click="UserId(user.id_chat)">
        <div  class="users__img"><img src="http://localhost:8081/uploads/user(1).png" alt="user" class="user-img"></div>
